...
def fetch_interviews_with_status_3():
    ...
    interviews = [dict(row) for row in cursor.fetchall()]   #每一行转为字典返回，所有字典又放在一个列表中
    conn.close()
    return interviews
...
def fetch_interview_questions(interview_id):
    ...
    questions = [dict(row) for row in cursor.fetchall()]  #字典组成的列表
    conn.close()
    return questions
...
def generate_pdf_report(model_output):
    ...
    try:
        ...
        #把渲染成功的html保存到文件，以检查其内容是否正的确
        # html_debug_path = r'D:\myProject\Practical_projects\project-interview\docs\interview_report.html'
        # with open(html_debug_path, 'w', encoding='utf-8') as f:
        #    f.write(rendered_html)
        
        print("4. 开始生成PDF...")

        #pdf_path = r'D:\myProject\interview_report.pdf'
        ...
        try:
           ...
           # HTML(string=rendered_html).write_pdf(pdf_path)
            ...
            print(f"错误类型: {type(pdf_error).__name__}")
            import traceback
            traceback.print_exc()
            pdf_bytes = None
...
def process_pending_reports():
    ...
    for interview in interviews:
        try:
            ...
            #print("-----model_output:-----",model_output)
            # Generate PDF report
            print(f"开始为面试ID {interview_id} 生成PDF报告...")
            pdf_report = generate_pdf_report(model_output)
            
            if pdf_report is None:
                print(f"警告: 面试ID {interview_id} 的PDF报告生成失败，跳过数据库更新")
                continue
                
            print(f"PDF报告生成成功，准备更新数据库...")
