...
def get_pending_interviews():
    ...
    conn.close()
    print("--aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa------",interviews)
    return interviews
...
def generate_questions(resume_content, position_name, requirements, responsibilities):
    ...
    print("resume_text:", resume_text) # 输出使用PyPDF2读取的简历内容，很长很长，做了分页展示
    ...
    try:
        response = client.chat.completions.create(
            #model="gpt-4", # 用的知乎老师给的1000次token的key， 生成面试题时生成的json会多一层question，代码会报错
            #model="glm-4-plus",  #用的知乎课上一个同学的key， 生成的json也没有多一层question
            model="qwen-plus",  #用的百炼的key， 生成的json也没有多一层question
            messages=[
                {"role": "system", "content": "你是一名专业的招聘面试官，请根据岗位要求和候选人简历生成3个针对性的技术面试问题，每个问题附带评分标准,返回标准的json格式。"},
                {"role": "user", "content": f"岗位名称: {position_name}\n岗位要求: {requirements}\n岗位职责: {responsibilities}\n候选人简历: {resume_text}\n\n请生成3个面试问题和评分标准，JSON格式参考 {json_format} ，每个问题满分10分。"}
            ],
        ...
        questions = json.loads(questions_json)  #转为字典
...
def save_questions(interview_id, questions):
    ...
    for question in questions:  # questions是列表，遍历生成的N个问题，循环插入到db,每个interview_id下面插入N个问题，将问题和评分标准分别存储
        ...
        #疑问：这里为什么question['question']不进行json.dumps处理，直接存字符串就行？
        cursor.execute('''
            INSERT INTO interview_questions (interview_id, question, score_standard)
            VALUES (?, ?, ?)
        ''', (interview_id, question['question'], score_standard))  #这里插入的时候，没有插入question_id,让它自增的。
...
def run_scheduler():
    ...
    while True:
        ...
        time.sleep(3)  #本来是1，我改成了3，给我的笔记本cpu减少点消耗，哈哈
