...
def generate_token(length=32):
    ...
    token = ''.join(secrets.choice(alphabet) for _ in range(length)) # 通过列表推导式生成 length个随机字符
    return token

def get_db():
    conn = sqlite3.connect('interview_system.db')
    conn.row_factory = sqlite3.Row  #设置为字典类型游标格式
    return conn
...
def get_candidates():
    ...
    candidates = [dict(row) for row in cursor.fetchall()]   #每一行转为一个字典，列表里面存了很多个字典
    conn.close()
...
def create_candidate():
    ...
    #将简历内容转换为 SQLite 支持的二进制格式（ sqlite3.Binary ），以便存储到数据库。
...
def download_resume(id):
    ...
    if resume and resume['resume_content']:
        return send_file(BytesIO(resume['resume_content']), download_name=f'resume_{id}.pdf', as_attachment=True)  #flask的函数，用于将文件发送给客户端（浏览器）
                        # BytesIO() 将二进制数据包装成类文件对象，这样Flask可以像处理文件一样处理这些数据
...
def create_interview():
    ...
    conn.commit()
    conn.close()
    return jsonify({'status': 'success'})  #虽然前端没有使用这个返回结果，但保留它仍然是一个良好的实践，便于前端调试和错误排查，便于未来扩展
...
def submit_answer(token):
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
        ...
        temp_file_path = temp_file.name         #文件名是随机的，这里不用关心
    ...
    #注意这里的where条件很关键，这里是根据 question_id 和 interview_id 来更新对应的问题答案，question_id是从前面获取过来的。
    conn.execute('''
    ...
    if not next_question:
        ...
        result = {
            "status": "success",
            "message": "答案已提交",
            "next_question": dict(next_question)   # 这里返回给前端axios.post方法的response
        }
